...
*Setup instructions for a Gitian build of MintCoin using a Debian VM or physical system.*
...
- [Building MintCoin](#building-mintcoin-core)
...
Clone the git repositories for MintCoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/mintcoinpay/mintcoin
git clone https://github.com/mintcoinpay/gitian.sigs.git
```
...
in the MintCoin repository under 'Fetch and create inputs' to install sources which require
...
To build MintCoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#setup-and-perform-gitian-builds) in the MintCoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/mintcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/mintcoinpay/mintcoin
...
For example:
```bash
URL=https://github.com/crowning-/mintcoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit mintcoin=${COMMIT} --url mintcoin=${URL} ../mintcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit mintcoin=${COMMIT} --url mintcoin=${URL} ../mintcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit mintcoin=${COMMIT} --url mintcoin=${URL} ../mintcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the mintcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=bionic on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../mintcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/mintcoinpay/mintcoin-detached-sigs.git

BTCPATH=/some/root/path/mintcoin
SIGPATH=/some/root/path/mintcoin-detached-sigs

./bin/gbuild --url mintcoin=${BTCPATH},signature=${SIGPATH} ../mintcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/mintcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/mintcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/mintcoin-osx-build.assert
```
...
[mintcoin/gitian.sigs](https://github.com/mintcoinpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
